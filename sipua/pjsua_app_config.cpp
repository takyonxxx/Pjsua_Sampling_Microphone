...
const char *stdout_refresh_text = "STDOUT_REFRESH";
...
static pj_status_t parse_args(int argc, char *argv[], pj_str_t *uri_to_call)
{
    ...
    struct pj_getopt_option long_options[] = { { "config-file", 1, 0,
            ...
            { "use-ims", 0, 0, OPT_USE_IMS }, { "id", 1, 0, OPT_ID }, {
                    ...
                    OPT_AUTO_UPDATE_NAT }, { "disable-stun", 0, 0,
                    OPT_DISABLE_STUN }, { "use-compact-form", 0, 0,
...
#if defined(PJMEDIA_HAS_SRTP) && (PJMEDIA_HAS_SRTP != 0)
            { "use-srtp", 1, 0, OPT_USE_SRTP }, { "srtp-secure", 1, 0,
                    OPT_SRTP_SECURE },
#endif
            ...
            { "vcapture-dev", 1, 0, OPT_VCAPTURE_DEV }, { "vrender-dev", 1, 0,
                    ...
                    "auto-play-avi", 0, 0, OPT_AUTO_PLAY_AVI }, { "use-cli", 0,
                    0, OPT_USE_CLI }, { "cli-telnet-port", 1, 0,
                    OPT_CLI_TELNET_PORT }, { "no-cli-console", 0, 0,
                    OPT_DISABLE_CLI_CONSOLE }, { NULL, 0, 0, 0 } };
    pj_status_t status;
    ...
    while ((c = pj_getopt_long(argc, argv, "", long_options, &option_index))
		...
		(void)file; }
            ...
            case OPT_USERNAME: /* Default authentication user */ {
                ...
                cur_acc->cred_info[cur_acc->cred_count].scheme = pj_str(
                        (char*)cstr);
                /*cur_acc->cred_info[cur_acc->cred_count].scheme = pj_str(
                        "Digest");*/
                }
            ...
            case OPT_REALM: /* Default authentication realm. */
                cur_acc->cred_info[cur_acc->cred_count].realm = pj_str(
                        pj_optarg);
                break;
            ...
            case OPT_PASSWORD: /* authentication password */
                ...
                cur_acc->cred_info[cur_acc->cred_count].data = pj_str(
                        pj_optarg);
            ...
            case OPT_USE_ICE:
                cfg->media_cfg.enable_ice = cur_acc->ice_cfg.enable_ice =
                        PJ_TRUE;
                break;
            ...
            case OPT_ICE_MAX_HOSTS:
                cfg->media_cfg.ice_max_host_cands =
                        cur_acc->ice_cfg.ice_max_host_cands = my_atoi(
                                pj_optarg);
                break;

            case OPT_ICE_NO_RTCP:
                cfg->media_cfg.ice_no_rtcp = cur_acc->ice_cfg.ice_no_rtcp =
                        PJ_TRUE;
                break;
            ...
            case OPT_USE_SRTP:
                ...
                if (!pj_isdigit(*pj_optarg) || app_config.cfg.use_srtp > 3) {
                    PJ_LOG(1,
                            (THIS_FILE, "Invalid value for --use-srtp option"));
                    return -1;
                }
            ...
            case OPT_THREAD_CNT:
                ...
                if (cfg->cfg.thread_cnt > 128) {
                    PJ_LOG(1,
                            (THIS_FILE, "Error: invalid --thread-cnt option"));
                    return -1;
                }
                break;
            ...
            case OPT_MAX_CALLS:
                ...
                if (cfg->cfg.max_calls
                        < 1|| cfg->cfg.max_calls > PJSUA_MAX_CALLS) {
                    PJ_LOG(1,
                ...
                case OPT_TLS_CIPHER:
                {
                    ...
                    if (pj_ansi_strnicmp(pj_optarg, "0x", 2) == 0) {
                        ...
                        cipher = (pj_ssl_cipher)atoi(pj_optarg);
                    }
