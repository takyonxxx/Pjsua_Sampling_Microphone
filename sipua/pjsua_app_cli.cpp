...
PJ_DEF(void) cli_get_info(char *info, pj_size_t size)
{
    ...
    pj_ansi_snprintf(info, size, "Telnet to %.*s:%d",
            (int) telnet_info.ip_address.slen, telnet_info.ip_address.ptr,
            telnet_info.port);
}
...
pj_status_t cli_init()
{
    ...
    do {
        ...
        const char *cstr = "pjsua_cli";
        cfg->name = pj_str((char *)cstr);
        /*cfg->title = pj_str("Pjsua CLI Application");*/
        cstr = "Pjsua CLI Application";
        cfg->title = pj_str((char*)cstr);
        ...
        if (app_config.cli_cfg.cli_fe & CLI_FE_CONSOLE) {
            ...
            const char *cstr = "shutdown";
            fe_cfg->quit_command = pj_str((char*)cstr);
...
{
    const char *cstr = "Invalid input\n";
    static const pj_str_t err_invalid_input = { (char*)cstr, 15 };
    ...
    if (param->cnt < param->max_cnt) {
        ...
        for (i = 0; i < count; ++i) {
            pj_ansi_snprintf(codec_id, sizeof(codec_id), "%.*s",
                    (int) c[i].codec_id.slen, c[i].codec_id.ptr);
...
static pj_status_t cmd_add_account(pj_cli_cmd_val *cval)
{
    ...
    const char *cstr = "Digest";
    acc_cfg.cred_info[0].scheme = pj_str((char*)cstr);
...
static pj_status_t cmd_show_account(pj_cli_cmd_val *cval)
{
    ...
    unsigned count = PJ_ARRAY_SIZE(acc_ids);
    int i;
    const char *cstr = "Account list:\n";
    static const pj_str_t header = { (char*)cstr, 15 };
...
static pj_status_t cmd_send_im(pj_cli_cmd_val *cval)
{
    ...
    if (result.nb_result != PJSUA_APP_NO_NB) {
        ...
        if (result.nb_result == -1) {
	    const char *cstr = "you can't send broadcast im like that!\n";
            static const pj_str_t err_msg = { (char*)cstr, 40 };
...
static pj_status_t cmd_subs_pres(pj_cli_cmd_val *cval, pj_bool_t subscribe)
{
    ...
    if (result.nb_result != PJSUA_APP_NO_NB) {
	    ...
	    const char *cstr = "Sorry, can only subscribe to" "buddy's presence, not from " "existing call\n";
            static const pj_str_t err_msg = { (char*)cstr, 71 };
        ...
        const char *cstr = "Sorry, can only subscribe to " "buddy's presence, not arbitrary " "URL (for now)\n";
        static const pj_str_t err_msg = { (char*)cstr, 76 };
        /*static const pj_str_t err_msg = { "Sorry, can only subscribe to "
...
static pj_status_t cmd_show_buddy(pj_cli_cmd_val *cval)
{
    ...
    int i;
    unsigned count = PJ_ARRAY_SIZE(ids);
    const char *cstr = "Buddy list:\n";
    static const pj_str_t header = { (char*)cstr, 13 };
    ...
    if (count == 0) {
        ...
        for (i = 0; i < (int) count; ++i) {
            ...
            pj_ansi_snprintf(out_str, sizeof(out_str), " [%2d] <%.*s>  %.*s\n",
                    ...
                    info.status_text.ptr, (int) info.uri.slen, info.uri.ptr);
...
static pj_status_t cmd_media_list(pj_cli_cmd_val *cval)
{
    unsigned i, count;
    pjsua_conf_port_id id[PJSUA_MAX_CONF_PORTS];
    const char *cstr = "Conference ports:\n";
    static const pj_str_t header = { (char*)cstr, 19 };
...
static pj_status_t cmd_media_connect(pj_cli_cmd_val *cval, pj_bool_t connect)
{
    ...
    if (status == PJ_SUCCESS) {
        const char *cstr = "Success\n";
        static const pj_str_t success_msg = { (char*)cstr, 9 };
        /*static const pj_str_t success_msg = { "Success\n", 9 };*/
        pj_cli_sess_write_msg(cval->sess, success_msg.ptr, success_msg.slen);
    }
    else {
        const char *cstr = "ERROR!!\n";
        static const pj_str_t err_msg = { (char*)cstr, 9 };
...
pj_status_t cmd_media_handler(pj_cli_cmd_val *cval)
{
    ...
    switch (pj_cli_get_cmd_id(cval->cmd)) {
        ...
        case CMD_MEDIA_SPEAKER_TOGGLE: {
            ...
            if (status != PJ_SUCCESS) {
                PJ_PERROR(2,
                        (THIS_FILE, status, "Warning: unable to retrieve route setting"));
            }
            ...
            PJ_LOG(4,
                    (THIS_FILE, "Setting output route to %s %s", (route==PJMEDIA_AUD_DEV_ROUTE_DEFAULT? "default" : "loudspeaker"), (status? "anyway" : "")));
...
static pj_status_t cmd_show_config()
{
    ...
    if (len < 1)
        ...
        PJ_LOG(3,
                (THIS_FILE, "Dumping configuration (%d bytes):\n%s\n", len, settings));
...
static pj_status_t cmd_make_single_call(pj_cli_cmd_val *cval)
{
    ...
    if (result.nb_result != PJSUA_APP_NO_NB) {
        ...
        if (result.nb_result == -1 || result.nb_result == 0) {
            const char *cstr = "You can't do that with make call!\n";
            static const pj_str_t err_msg = { (char*)cstr, 35 };
...
static pj_status_t cmd_answer_call(pj_cli_cmd_val *cval)
{
    ...
    if (current_call == PJSUA_INVALID_ID || call_info.role != PJSIP_ROLE_UAS
        ...
        const char *cstr = "No pending incoming call\n";
        static const pj_str_t err_msg = { (char*)cstr, 26 };
        ...
        int st_code;
        char contact[120];
        const char *cstr = "Contact";
        pj_str_t hname = { (char*)cstr, 7 };
        ...
        if (st_code / 100 == 3) {
            if (cval->argc < 3) {
                const char *cstr = "Enter URL to be put " "in Contact\n";
                static const pj_str_t err_msg = { (char*)cstr, 32 };
         ...
         */
        if (current_call == PJSUA_INVALID_ID) {
            const char *cstr = "Call has been disconnected\n";
            static const pj_str_t err_msg =
                    { (char*)cstr, 28 };
            /*static const pj_str_t err_msg =
...
{
    if (current_call == PJSUA_INVALID_ID) {
        const char *cstr = "No current call\n";
        static const pj_str_t err_msg = { (char*)cstr, 17 };
...
static pj_status_t cmd_next_call(pj_bool_t next)
{
    ...
    if (current_call != PJSUA_INVALID_ID) {
        ...
        pjsua_call_get_info(current_call, &call_info);
        PJ_LOG(3,
                (THIS_FILE,"Current dialog: %.*s", (int)call_info.remote_info.slen, call_info.remote_info.ptr));
...
static pj_status_t cmd_transfer_call(pj_cli_cmd_val *cval)
{
    if (current_call == PJSUA_INVALID_ID) {
        ...
        const char *cstr = "Refer-Sub";
        pj_str_t STR_REFER_SUB = { (char*)cstr, 9 };
        /*pj_str_t STR_REFER_SUB = { "Refer-Sub", 9 };*/
        cstr = "false";
        pj_str_t STR_FALSE = { (char*)cstr, 5 };
        ...
        if (result.nb_result != PJSUA_APP_NO_NB) {
            if (result.nb_result == -1 || result.nb_result == 0) {
                const char *cstr = "You can't do that with " "transfer call!\n";
                static const pj_str_t err_msg = { (char*)cstr, 39 };
...
static pj_status_t cmd_transfer_replace_call(pj_cli_cmd_val *cval)
{
    if (current_call == -1) {
        ...
        pj_str_t tmp = pj_str(buf);
        unsigned count;
        cstr = "Invalid destination call number\n";
        static const pj_str_t err_invalid_num = { (char*)cstr, 32 };
        ...
        if (count <= 1) {
            const char *cstr = "There are no other calls\n";
            static const pj_str_t err_no_other_call = { (char*)cstr, 25 };
        ...
        if (call != current_call) {
            const char *cstr = "Call has been disconnected\n";
            static pj_str_t err_call_dc = { (char*)cstr, 27 };
        ...
        if (dst_call == call) {
            const char *cstr = "Destination call number must not be the " "same as the call being transferred\n";
            static pj_str_t err_same_num = { (char*)cstr, 74 };
        ...
        PJSUA_XFER_NO_REQUIRE_REPLACES, &msg_data_);
    }
    return PJ_SUCCESS;
}
...
static pj_status_t cmd_dtmf_2833(pj_cli_cmd_val *cval)
{
    if (current_call == PJSUA_INVALID_ID) {
        ...
        if (status != PJ_SUCCESS) {
            ...
            const char *cstr = "DTMF digits enqueued " "for transmission\n";
            static const pj_str_t msg = { (char*)cstr, 39 };
...
static pj_status_t cmd_call_info(pj_cli_cmd_val *cval)
{
    if (current_call == PJSUA_INVALID_ID) {
        ...
        const char *cstr = "INFO";  
        const pj_str_t SIP_INFO = pj_str((char*)cstr);
        ...
        for (i = 0; i < cval->argv[1].slen; ++i) {
            ...
            pjsua_msg_data_init(&msg_data);
            const char *cstr = "application/dtmf-relay";
            msg_data.content_type = pj_str((char*)cstr);
            /*msg_data.content_type = pj_str("application/dtmf-relay");*/

            pj_ansi_snprintf(body, sizeof(body), "Signal=%c\n"
                    "Duration=160", cval->argv[1].ptr[i]);
...
static pj_status_t cmd_send_arbitrary(pj_cli_cmd_val *cval)
{
    if (pjsua_acc_get_count() == 0) {
        const char *cstr = "Sorry, need at least one " "account configured\n";
        static const pj_str_t err_msg = { (char*)cstr, 45 };
        ...
        pj_str_t tmp = pj_str(dest);
        struct input_result result;
        const char *cstr = "Send arbitrary request to " "remote host\n";
        static const pj_str_t header = { (char*)cstr, 39 };
        ...
        if (result.nb_result != PJSUA_APP_NO_NB) {
            if (result.nb_result == -1) {
                const char *cstr = "Sorry you can't do that!\n";
                static const pj_str_t err_msg = { (char*)cstr, 26 };
...
static pj_status_t add_call_command(pj_cli_t *c)
{
    ...
    pj_str_t xml = pj_str((char*)call_command);
    return pj_cli_add_cmd_from_xml(c, NULL, &xml, cmd_call_handler,
    NULL, get_choice_value);
}

static pj_status_t add_presence_command(pj_cli_t *c)
{
    const char* presence_command =
    ...
    pj_str_t xml = pj_str((char*)presence_command);

    return pj_cli_add_cmd_from_xml(c, NULL, &xml, cmd_presence_handler,
    NULL, get_choice_value);
}
...
static pj_status_t add_account_command(pj_cli_t *c)
{
    ...
    pj_str_t xml = pj_str((char*)account_command);
    return pj_cli_add_cmd_from_xml(c, NULL, &xml, cmd_account_handler,
    NULL, get_choice_value);
}

static pj_status_t add_media_command(pj_cli_t *c)
{
    const char* media_command =
    ...
    pj_str_t xml = pj_str((char*)media_command);
    return pj_cli_add_cmd_from_xml(c, NULL, &xml, cmd_media_handler,
    NULL, get_choice_value);
}
...
static pj_status_t add_config_command(pj_cli_t *c)
{
    ...
    pj_str_t xml = pj_str((char*)config_command);
    return pj_cli_add_cmd_from_xml(c, NULL, &xml, cmd_config_handler,
    NULL, get_choice_value);
}
...
static pj_status_t add_other_command(pj_cli_t *c)
{
    ...
    const char* ip_change_command =
            "<CMD name='ip_change' id='130' desc='Handle IP change'/>";

    pj_status_t status;
    pj_str_t sleep_xml = pj_str((char*)sleep_command);
    pj_str_t network_xml = pj_str((char*)network_command);
    pj_str_t shutdown_xml = pj_str((char*)shutdown_command);
    pj_str_t restart_xml = pj_str((char*)restart_command);
    pj_str_t ip_change_xml = pj_str((char*)ip_change_command);
