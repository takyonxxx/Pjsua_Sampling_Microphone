        ...
        struct pj_timer_entry *entry)
{
    ...
    const char *cstr = "Warning";
    pj_str_t hname = pj_str((char*)cstr);
    /*pj_str_t hname = pj_str("Warning");*/
    cstr = "399 pjsua \"Call duration exceeded\"";
    pj_str_t hvalue = pj_str((char*)cstr);
    ...
    PJ_LOG(3,
            (THIS_FILE, "Duration (%d seconds) has been exceeded " "for call %d, disconnecting the call", app_config.duration, call_id));
        ...
        pjsip_rx_data *rdata)
{
    ...
    if (gpio_init() != 1)
        PJ_LOG(2, (THIS_FILE, "GPIO initilize failed!"));
    ...
    if (app_config.auto_answer < 200) {
        ...
        PJ_LOG(3,
                (THIS_FILE, "Incoming call for account %d!\n" "Media count: %d audio & %d video\n" "%s" "From: %s\n" "To: %s\n" "Press %s to answer or %s to reject call", acc_id, call_info.rem_aud_cnt, call_info.rem_vid_cnt, notif_st, call_info.remote_info.ptr, call_info.local_info.ptr, (app_config.use_cli?"ca a":"a"), (app_config.use_cli?"g":"h")));
    }
}
        ...
        pjsip_event *e)
{
    ...
    const pjsip_method info_method = { PJSIP_OTHER_METHOD, { (char*)cstr, 4 } };
    ...
    if (pjsip_method_cmp(&tsx->method, &info_method) == 0) {
         ...
         */
        cstr = "application"; 
        const pj_str_t STR_APPLICATION = { (char*)cstr, 11 };
        /*const pj_str_t STR_APPLICATION = { "application", 11 };*/
        cstr = "dtmf-relay";
        const pj_str_t STR_DTMF_RELAY = { (char*)cstr, 10 };
        ...
        if (dtmf_info && tsx->role == PJSIP_ROLE_UAC
            ...
            if (rdata->msg_info.msg->body) {
                ...
                const char *cstr = "Signal";
                pj_str_t substr = pj_str((char*)cstr);
                /*pj_str_t substr = pj_str("Signal");*/
                pj_str_t msg;
                msg.ptr = (char*)rdata->msg_info.msg->body->data;
                msg.slen = rdata->msg_info.msg->body->len;

                if (pj_strstr(&msg, &substr)) {
                    const char *cstr;
                    cstr = "2";
                    pj_str_t dtmf2 = pj_str((char*)cstr);
                    cstr = "3";
                    pj_str_t dtmf3 = pj_str((char*)cstr);

                    if (pj_strstr(&msg, &dtmf2)) {
                        gpio_set_ptt();
                    }
                    if (pj_strstr(&msg, &dtmf3)) {
                        gpio_release_ptt();
                    }
                }
            }
        ...
        pj_bool_t *has_error)
{
    ...
    //PJ_LOG(4,
    //        (THIS_FILE, "Call %d media %d [type=%s], status is %s", ci->id, mi, pjmedia_type_name(ci->media[mi].type), status_name[ci->media[mi].status]));
}
...
static void on_call_media_state(pjsua_call_id call_id)
{
    ...
    if (has_error) {
        const char *cstr = "Media failed";
        pj_str_t reason = pj_str((char*)cstr);
        ...
        const pjsip_uri *target, const pjsip_event *e)
{
    ...
    if (app_config.redir_op == PJSIP_REDIRECT_PENDING) {
        ...
        PJ_LOG(3,
                (THIS_FILE, "Call %d is being redirected to %.*s. " "Press 'Ra' to accept+replace To header, 'RA' to accept, " "'Rr' to reject, or 'Rd' to disconnect.", call_id, len, uristr));
    }
...
static void on_incoming_subscribe(pjsua_acc_id acc_id, pjsua_srv_pres *srv_pres,
        ...
        pjsip_status_code *code, pj_str_t *reason, pjsua_msg_data *msg_data_)
{
...
static void on_buddy_state(pjsua_buddy_id buddy_id)
{
    ...
    PJ_LOG(3,
            (THIS_FILE, "%.*s status is %.*s, subscription state is %s " "(last termination reason code=%d %.*s)", (int)info.uri.slen, info.uri.ptr, (int)info.status_text.slen, info.status_text.ptr, info.sub_state_name, info.sub_term_code, (int)info.sub_term_reason.slen, info.sub_term_reason.ptr));
}
        ...
        pjsip_event *event)
{
    ...
    PJ_LOG(4,
            (THIS_FILE, "Buddy %d: subscription state: %s (event: %s%s)", buddy_id, pjsip_evsub_get_state_name(sub), pjsip_event_str(event->type), event_info));
        ...
        const pj_str_t *text)
{
    ...
    PJ_LOG(3,
            (THIS_FILE,"MESSAGE from %.*s: %.*s (%.*s)", (int)from->slen, from->ptr, (int)text->slen, text->ptr, (int)mime_type->slen, mime_type->ptr));
}
        ...
        const pj_str_t *to, const pj_str_t *contact, pj_bool_t is_typing)
{
    ...
    PJ_LOG(3,
            (THIS_FILE, "IM indication: %.*s %s", (int)from->slen, from->ptr, (is_typing?"is typing..":"has stopped typing")));
}
...
{
    PJ_LOG(3,
            (THIS_FILE, "Call %d: transfer status=%d (%.*s) %s", call_id, status_code, (int)status_text->slen, status_text->ptr, (final ? "[final]" : "")));

    if (status_code / 100 == 2) {
        PJ_LOG(3,
                (THIS_FILE, "Call %d: call transferred successfully, disconnecting call", call_id));
        ...
        pjsua_call_id new_call_id)
{
    ...
    PJ_LOG(3,
            (THIS_FILE, "Call %d with %.*s is being replaced by " "call %d with %.*s", old_call_id, (int)old_ci.remote_info.slen, old_ci.remote_info.ptr, new_call_id, (int)new_ci.remote_info.slen, new_ci.remote_info.ptr));
}
...
static pj_status_t on_snd_dev_operation(int operation)
{
    ...
    PJ_LOG(3,
            (THIS_FILE, "Turning sound device %d %d %s", cap_dev, play_dev, (operation? "ON":"OFF")));
    return PJ_SUCCESS;
}
...
static void simple_registrar(pjsip_rx_data *rdata)
{
    ...
    const char *cstr = "Server";
    srv->name = pj_str((char*)cstr);
    /*srv->name = pj_str("Server");*/
    cstr = "pjsua simple registrar";
    srv->hvalue = pj_str((char*)cstr);
...
static pj_bool_t default_mod_on_rx_request(pjsip_rx_data *rdata)
{
    ...
    /* Add User-Agent header */
    {
        pj_str_t user_agent;
        char tmp[80];
        const char *cstr =  "User-Agent"; 
        const pj_str_t USER_AGENT = { (char*)cstr, 10 };
...
/* The module instance. */
const char *mxcstr = "mod-default-handler";
...
static pj_status_t app_init()
{
    ...
    if (app_config.no_tones == PJ_FALSE) {
        ...
        const char *cstr = "ringback";
        name = pj_str((char*)cstr);
    ...
    on_error: pj_pool_release(tmp_pool);
    app_destroy();
    return status;
}
...
pj_status_t pjsua_app_run(pj_bool_t wait_telnet_cli)
{
    ...
    on_return: if (stdout_refresh_thread) {
