...
static void ui_send_dtmf_info()
{
    if (current_call == -1) {
        ...
        for (i = 0; i < digits.slen; ++i) {
            ...
            msg_data_.content_type = pj_str((char*)cstr);
...
static void ui_change_online_status()
{
    ...
    const char *pcstr[] = { "Available", "Busy", "On the phone", "Idle", "Away" , "Be right back", "Offline" };
    struct opt {
        int id;
        char *name;
    } opts[] = { { AVAILABLE, (char*)pcstr[0] }, { BUSY, (char*)pcstr[1] }, { OTP,
            (char*)pcstr[2] }, { IDLE, (char*)pcstr[3] }, { AWAY, (char*)pcstr[4] }, { BRB,
            (char*)pcstr[5] }, { OFFLINE, (char*)pcstr[6] } };
    /*struct opt {
        int id;
        char *name;
...
static void ui_conf_connect(char menuin[])
{
    ...
    if (cnt != 3) {
        ...
        src_title = (
                menuin[1] == 'c' ?
                        "Connect src port #" : "Disconnect src port #");
